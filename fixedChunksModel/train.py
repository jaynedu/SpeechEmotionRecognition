...
from fixedChunksModel.graph import model as Model
...
with tf.get_default_graph().as_default() as graph:
    # 加载数据
    train_iterator = utils.tfrecord.read_tfrecord(args.train_path, args.epoch, args.train_batch, True)
    train_x, train_y = train_iterator.get_next()
    val_iterator = utils.tfrecord.read_tfrecord(args.val_path, -1, args.val_batch, False)
    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        try:
            ...
            for _i in tbar:
                traindataBatch, trainlabelBatch, trainx, trainy = sess.run([train_x, train_y])
